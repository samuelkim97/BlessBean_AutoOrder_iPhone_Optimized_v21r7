...
export default function AutoOrderAppV15_3() {
  ...
  const [undo, setUndo] = useState<{ item: CartItem; index: number; expiresAt: number } | null>(null);
  ...
  useEffect(() => {
    if (!fileDate) return;
    try {
      localStorage.setItem(LS_MANUAL_KEY, JSON.stringify({ fileDate, manualPrices, savedAt: Date.now() }));
    } catch {}
  }, [manualPrices, fileDate]);

  // ===== 최근 거래처 / 주문 저장 =====
  useEffect(() => {
    try { localStorage.setItem(LS_CLIENT_HISTORY, JSON.stringify(clientHistory.slice(0, MAX_RECENT_CLIENTS))); } catch {}
  }, [clientHistory]);

  useEffect(() => {
    try { localStorage.setItem(LS_ORDER_HISTORY, JSON.stringify(orderHistory.slice(0, MAX_RECENT_ORDERS))); } catch {}
  }, [orderHistory]);
  ...
  useEffect(() => {
    ...
    const lines = cart.filter((i) => i.quantity > 0)
  ...
  const pushRecentClient = useCallback((clientName: string) => {
    ...
    if (!name) return;
    const now = Date.now();
    setClientHistory((prev) => [{ name, lastUsedAt: now }, ...prev.filter((x) => x.name !== name)].slice(0, MAX_RECENT_CLIENTS));
  }, []);
...
((id: string) => {
  ...
  const handleExcelUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const sheet of wb.SheetNames) {
      ...
      let nameIdx = -1; let priceIdx = -1; const countryIdx = 1;
      let headerRow = -1;
      for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const hasNameHeader = row.some((c) => typeof c === "string" && /(품명|제품명)/.test(c));
        const hasPriceHeader = row.some((c) => typeof c === "string" && /(단가|가격)/.test(c));
      ...
      for (let j = headerRow + 1; j < rows.length; j++) {
        ...
        if (typeof name === "string" && name && price && currentCountry) {
          ...
          if (!Number.isNaN(priceNum)) collected.push({ country: currentCountry, name: name.trim(), price: priceNum, priceGroup: sheet });
        }
      }
    }
    ...
    try {
      ...
      localStorage.setItem(LS_MANUAL_KEY, JSON.stringify({ fileDate: label, manualPrices: {}, savedAt: Date.now() }));
    } catch {}
    setStep(2);
    setToastMode("normal");
    setToast("단가표 로드 완료!");
  ...
  // ===== 국가 / 품목 =====
  const countries = useMemo(() => Array.from(new Set(itemsAll.filter((i) => i.priceGroup === priceGroup).map((i) => i.country))).sort((a, b) => a.localeCompare(b)), [itemsAll, priceGroup]);
  const items = useMemo(() => itemsAll.filter((i) => i.country === selectedCountry && i.priceGroup === priceGroup), [itemsAll, selectedCountry, priceGroup]);

  // ===== 장바구니 =====
  const addToCart = useCallback((item: PriceItem) => {
    const key = makePriceKey(item.priceGroup, item.country, item.name);
    const priceToUse = manualPrices[key] ?? item.price;
    setCart((prev) => {
      const exists = prev.find((x) => x.name === item.name && x.country === item.country && x.priceGroup === item.priceGroup);
      if (exists) return prev;
      return [...prev, { name: item.name, country: item.country, price: priceToUse, quantity: 0, priceGroup: item.priceGroup }];
    });
  }, [manualPrices]);

  const updateQty = useCallback((n: string, c: string, g: string, v: number) => {
    setCart((prev) => prev.map((x) => (x.name === n && x.country === c && x.priceGroup === g ? { ...x, quantity: Math.max(x.quantity + v, 0) } : x)));
  }, []);

  const setQtyExact = useCallback((n: string, c: string, g: string, qty: number) => {
    const safe = Number.isFinite(qty) ? qty : 0;
    setCart((prev) => prev.map((x) => (x.name === n && x.country === c && x.priceGroup === g ? { ...x, quantity: Math.max(safe, 0) } : x)));
  }, []);
  ...
  const removeCartItemWithUndo = useCallback((target: CartItem, index: number) => {
    setCart((prev) => prev.filter((x) => !(x.name === target.name && x.country === target.country && x.priceGroup === target.priceGroup)));
  ...
  const undoRemove = useCallback(() => {
    ...
    if (Date.now() > undo.expiresAt) {
      setUndo(null); setToastMode("normal"); setToast("되돌리기 시간이 지났습니다."); return;
    }
    setCart((prev) => {
      const exists = prev.some((x) => x.name === undo.item.name && x.country === undo.item.country && x.priceGroup === undo.item.priceGroup);
      if (exists) return prev;
      const next = [...prev]; const idx = Math.min(Math.max(undo.index, 0), next.length); next.splice(idx, 0, undo.item); return next;
    });
    setUndo(null); setToastMode("normal"); setToast("복구 완료!");
  }, [undo]);

  // ===== 단가 수동 수정 =====
  const editPrice = (target: CartItem) => {
    const key = makePriceKey(target.priceGroup, target.country, target.name);
    setEditingPriceKey(key); setPriceInput(String(target.price));
  };

  const applyPriceEdit = (target: CartItem) => {
    const cleaned = (priceInput ?? "").trim().replace(/[^0-9]/g, "");
    if (!cleaned) { setToastMode("normal"); setToast("숫자로 단가를 입력해주세요."); return; }
    const numeric = Number(cleaned);
    if (Number.isNaN(numeric)) { setToastMode("normal"); setToast("올바른 숫자를 입력해주세요."); return; }
    setCart((prev) => prev.map((x) => (x.name === target.name && x.country === target.country && x.priceGroup === target.priceGroup ? { ...x, price: numeric } : x)));
    const key = makePriceKey(target.priceGroup, target.country, target.name);
    setManualPrices((prev) => ({ ...prev, [key]: numeric }));
    setEditingPriceKey(null); setToastMode("normal"); setToast("단가가 수정되었습니다.");
  };

  const cancelPriceEdit = () => { setEditingPriceKey(null); setPriceInput(""); setToastMode("normal"); setToast("단가 변경이 취소되었습니다."); };

  // ===== 복사 =====
  const copyToClipboard = async () => {
    if (!message) { setToastMode("normal"); setToast("복사할 문구가 없습니다."); return; }
    try {
      await navigator.clipboard.writeText(message);
      setToastMode("normal"); setToast("문구 복사 완료!"); pushRecentClient(client); pushRecentOrder();
    } catch {
      const ta = document.createElement("textarea"); ta.value = message; document.body.appendChild(ta); ta.select(); document.execCommand("copy"); document.body.removeChild(ta);
      setToastMode("normal"); setToast("문구 복사 완료!"); pushRecentClient(client); pushRecentOrder();
    }
  };
  ...
  // ===== 거래처 입력 단계로 리셋 =====
  const backToClient = () => {
    setStep(2); setClient(""); setCart([]); setSelectedCountry(null); setNoteType(null); setSmallPack(false); setSameDay(false); setMessage(""); setUndo(null); setToastMode("normal"); setToast("초기화 완료");
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-lg">
      ...
      <main className="flex-1 px-3 pb-28">
          ...
          <div className="mt-5 space-y-3">
              ...
              <div className="mt-1">
                ...
                <div className="flex flex-wrap gap-2 justify-center mt-2">
                  {clientHistory.slice(0, MAX_RECENT_CLIENTS).map((x) => (
                    <button key={x.name} onClick={() => { setClient(x.name); setToastMode("normal"); setToast("거래처 선택됨"); }} className="px-3 py-2 rounded-full border border-gray-300 bg-gray-50 text-gray-700 text-sm active:scale-95" title={x.name}>{x.name}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-4 gap-2">
              {SALESPEOPLE.map((name) => (
                <button key={name} onClick={() => setSender(name)} className={`py-2 rounded-md border text-sm ${sender === name ? "bg-red-600 text-white border-red-600" : "bg-red-100 text-red-800 border-red-300"}`}>{name}</button>
              ))}
            </div>
              ...
              <div className="mt-2">
                ...
                <div className="mt-2 space-y-2">
                  {filteredOrderHistory.slice(0, 5).map((o) => {
                    ...
                    const timeText = new Date(o.createdAt).toLocaleString("ko-KR", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });
                    const preview = itemCount === 0 ? "품목 없음" : itemCount === 1 ? `${o.cart[0].country} ${o.cart[0].name}` : `${o.cart[0].country} ${o.cart[0].name} 외 ${itemCount - 1}개`;
                    const fileHint = o.fileDate && fileDate && o.fileDate !== fileDate ? " (단가표 다름)" : "";
                    return (
                      <button key={o.id} onClick={() => loadRecentOrder(o)} className="w-full text-left border border-gray-200 rounded-md p-3 bg-white active:scale-[0.99]">
                      ...
                      <div className="flex justify-end mt-1">
                        ...
                        <div className="text-xs text-gray-600 mt-1">{preview} · {totalKg}kg · 그룹 {o.priceGroup || "-"} · {o.sender}{fileHint}</div>
                      </button>
                    );
                  })}
                </div>
        ...
        {step === 3 && (
          <div className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              {["(1)", "(2)", "(3)", "(4)"].map((g) => (
                <button key={g} onClick={() => { setPriceGroup(g); setSelectedCountry(null); setStep(4); }} className="py-4 bg-red-100 border border-red-300 text-red-800 text-xl rounded-md">{g}</button>
              ))}
            </div>
          </div>
        )}
                ...
                {items.map((i) => {
                  ...
                  return (
                    <div key={`${i.priceGroup}-${i.country}-${i.name}`} className="flex items-center gap-2">
                      <button onClick={() => addToCart(i)} className="flex-1 justify-between bg-red-50 text-red-800 border border-red-300 px-4 py-3 rounded-md active:scale-95 flex items-center">
                        <span className="flex-1 pr-3 text-sm leading-snug" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }} title={i.name}>{i.name}</span>
                        <span className="text-base whitespace-nowrap">{displayPrice.toLocaleString()}원</span>
                      </button>
                      <button onClick={() => removeFromCart(i.name, i.country, i.priceGroup)} className="px-3 py-3 rounded-md border border-red-300 text-red-700" title="장바구니에서 삭제">❌</button>
                    </div>
                  );
                })}
              </div>
        ...
        {cart.length > 0 && (
          <div className="mt-6 mb-28">
            <p className="font-semibold text-red-700 flex items-center justify-between mb-2">🧺 장바구니
              <span className="flex gap-2">
                ...
                <button onClick={() => setSmallPack(!smallPack)} className={`text-xs px-3 py-2 rounded-md border ${smallPack ? "bg-green-600 text-white border-green-600" : "bg-green-100 text-green-800 border-green-300"}`}>5kg 소분</button>
                <button onClick={() => setSameDay(!sameDay)} className={`text-xs px-3 py-2 rounded-md border ${sameDay ? "bg-blue-600 text-white border-blue-600" : "bg-blue-100 text-blue-800 border-blue-300"}`}>금일</button>
              </span>
            </p>
            ...
            {cart.map((i, idx) => {
              ...
              const preview = (() => { const cleaned = priceInput.trim().replace(/[^0-9]/g, ""); if (!isEditing || !cleaned) return ""; const num = Number(cleaned); if (Number.isNaN(num)) return ""; return `${num.toLocaleString()}원`; })();
              return (
                <div key={`${i.priceGroup}-${i.country}-${i.name}`} className="bg-red-50 border border-red-200 rounded-lg p-3 mb-2">
                  ...
                  <div className="flex justify-between items-center mt-2">
                    <div className="flex gap-2 flex-wrap">
                      {[1,5,20].map((v) => (<button key={v} onClick={() => updateQty(i.name, i.country, i.priceGroup, v)} className="bg-red-200 text-red-800 text-sm px-4 py-2 rounded-md active:scale-95">+{v}</button>))}
                      <button onClick={() => updateQty(i.name, i.country, i.priceGroup, -i.quantity)} className="bg-gray-200 text-gray-800 text-sm px-4 py-2 rounded-md">0kg</button>
                      <button onClick={() => editPrice(i)} className="bg-white border border-red-300 text-red-700 text-sm px-3 py-2 rounded-md active:scale-95">단가</button>
                      <button onClick={() => removeCartItemWithUndo(i, idx)} className="bg-red-600 text-white text-sm px-4 py-2 rounded-md active:scale-95">삭제</button>
                    </div>
                    <div className="flex items-center gap-1">
                      <input value={String(i.quantity)} onChange={(e) => { const cleaned = e.target.value.replace(/[^0-9]/g, ""); const num = cleaned ? Number(cleaned) : 0; if (Number.isNaN(num)) return; setQtyExact(i.name, i.country, i.priceGroup, num); }} onFocus={(e) => e.currentTarget.select()} inputMode="numeric" pattern="[0-9]*" className="w-16 text-right border border-red-300 rounded-md px-2 py-1 text-[16px] bg-white" aria-label="수량(kg) 직접 입력" />
                  ...
                  {isEditing && (
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <input ref={priceInputRef} value={priceInput} onChange={(e) => setPriceInput(e.target.value.replace(/[^0-9]/g, ""))} onKeyDown={(e) => { if (e.key === "Enter") applyPriceEdit(i); if (e.key === "Escape") cancelPriceEdit(); }} inputMode="numeric" pattern="[0-9]*" className="w-32 text-right border border-red-300 rounded-md px-2 py-1 text-[16px]" placeholder="숫자만 입력" />
                      <button onClick={() => applyPriceEdit(i)} className="bg-red-600 text-white text-xs px-3 py-1 rounded-md active:scale-95">확인</button>
                      <button onClick={cancelPriceEdit} className="bg-gray-200 text-gray-700 text-xs px-3 py-1 rounded-md active:scale-95">취소</button>
      ...
      <footer className="fixed left-4 right-4 z-50 pointer-events-none flex justify-between items-end" style={{ bottom: "calc(1rem + env(safe-area-inset-bottom))" }}>
      ...
      {toast && (
        <div className="fixed left-1/2 -translate-x-1/2 bg-red-600 text-white text-sm px-4 py-2 rounded-md shadow-md flex items-center gap-3" style={{ bottom: "calc(6rem + env(safe-area-inset-bottom))" }}>
          <span>{toast}</span>
          {toastMode === "undo" && undo && (
            <button onClick={undoRemove} className="underline font-semibold" aria-label="삭제 되돌리기">되돌리기</button>
          )}
        </div>
      )}
    </div>
