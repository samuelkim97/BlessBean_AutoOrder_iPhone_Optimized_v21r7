...
// BlessBean AutoOrder – iPhone 최적화 버전 최종 App.tsx
// - 담당자 선택 (김대용 / 최운호 / 김용준 / 이천복)
// - 인사말 커스터마이즈
// - (1)~(4) 단가 그룹 + 품목별 단가 수동 수정 (인라인 편집)
// - 5kg 소분 옵션 토글 (총 금액 아래 한 줄 띄워서 표시)
...
export default function AutoOrderAppV15_3() {
  ...
  // 품목별 수동 단가 (priceGroup + country + name 기준)
...
총 금액 ${total.toLocaleString()}원${smallPackText}${note}`
    );
  }, [cart, noteType, client, sender, smallPack]);

  // ===== 엑셀 업로드 (iPhone '파일' 앱 포함) =====
  const handleExcelUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          if (row.some((c) => typeof c === "string" && /(품명|제품명)/.test(c))) {
            ...
            for (let j = i + 1; j < rows.length; j++) {
              ...
              const maybe =
                typeof rawCountry === "string" ? normalizeCountry(rawCountry) : "";
      ...
      setManualPrices({}); // 새로운 단가표 로드 시 수동 단가 초기화
  ...
  const countries = useMemo(
    () =>
      Array.from(
        new Set(
          itemsAll.filter((i) => i.priceGroup === priceGroup).map((i) => i.country)
        )
      ),
    [itemsAll, priceGroup]
  );
  ...
  const items = useMemo(
    () =>
      itemsAll.filter(
        (i) => i.country === selectedCountry && i.priceGroup === priceGroup
      ),
  ...
  const updateQty = (n: string, c: string, g: string, v: number) => {
    setCart((prev) =>
      prev.map((x) =>
  ...
  const removeFromCart = (n: string, c: string, g: string) => {
    setCart((prev) =>
      prev.filter(
        (x) => !(x.name === n && x.country === c && x.priceGroup === g)
      )
    );
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white text-lg">
      <header className="sticky top-0 z-50 bg-white border-b border-red-200 p-3 text-center font-bold text-red-700 text-xl">
        ...
        {fileDate && (
          <p className="text-sm text-gray-600 mt-1">📅 {fileDate}</p>
        )}
      </header>
      ...
      <main className="flex-1 px-3 pb-28">
        ...
        {step === 2 && (
          <div className="mt-5 space-y-3">
            ...
            <p className="text-center text-red-700 font-semibold mt-2">
              담당자 선택
            </p>
                ...
                {items.map((i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 이 버튼은 장바구니에 담긴 동일 품목 제거용 */}
                      <button
                        onClick={() =>
                          removeFromCart(i.name, i.country, i.priceGroup)
                        }
          ...
          <div className="mt-6 mb-28">
            <p className="font-semibold text-red-700 flex items-center justify-between mb-2">
              ...
              <span className="flex gap-2">
                <button
                  onClick={() =>
                    setNoteType(noteType === "account" ? null : "account")
                  }
                ...
                <button
                  onClick={() =>
                    setNoteType(noteType === "card" ? null : "card")
                  }
                  className={`text-xs px-3 py-2 rounded-md border ${
                    noteType === "card"
            ...
            {cart.map((i) => (
              ...
              >
                <p className="text-base text-red-800 break-words leading-snug">
                  {i.country} {i.name} {i.quantity}kg * {""}
                  ...
                  <div className="flex gap-2">
                    {[1, 5, 20].map((v) => (
                      <button
                        key={v}
                        onClick={() =>
                          updateQty(i.name, i.country, i.priceGroup, v)
                        }
                ...
                {editingPriceKey ===
                  makePriceKey(i.priceGroup, i.country, i.name) && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      pattern="[0-9]*"
                      className="w-28 text-right border border-red-300 rounded-md px-2 py-1 text-sm"
                      placeholder="숫자만 입력"
                    />
                    <button
